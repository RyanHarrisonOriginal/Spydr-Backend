...
import type { ICommand } from "../../shared/application/command.js";
...
} from "../commands/project-child.commands.js";
...
export async function collectPreMutationProjectIds(
  ...
  if (command instanceof UpdateTaskCommand) {
    ...
    const item = await repositories.taskViews.getListItem(
      command.orgId,
      command.taskId
    );
  ...
  if (command instanceof DeleteNoteCommand) {
    const item = await repositories.noteViews.getListItem(
      command.orgId,
      command.noteId
    );
    return uniqueProjectIds([item?.project?.id]);
