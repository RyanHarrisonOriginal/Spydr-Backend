import type { SpydrNodeStatus, SpydrPriority, TaskStatus } from "../shared/models/shared.js";
import { isTaskStatus } from "../shared/models/shared.js";
...
export function defaultTaskDetailsFromSource(input: {
  now: Date;
  status?: SpydrNodeStatus | TaskStatus | string;
  projectDetails?: {
  ...
  const status = input.status ? toTaskStatus(input.status) : "active";
  return {
    dueDate: taskDetails?.dueDate ?? projectDetails?.targetDate ?? null,
    completedAt: (status === "completed" ? now : null) as Date | null,
    isBlocked: false,
