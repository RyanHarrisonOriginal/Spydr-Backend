...
describe("ApplyActiveNoteCommandHandler", () => {
  ...
  describe("review snapshot merge", () => {
    it("keeps previously applied operations when applying remaining ones", async () => {
      ...
      handler = new ApplyActiveNoteCommandHandler(commandBus, sessions);

      await handler.execute(
        new ApplyActiveNoteCommand("user-1", "org-1", {
          activeNoteId: "session-1",
          content: "Throw more teeps",
          operations: [
            createTaskOperation("op-1", { selected: false }),
            createNoteOperation("op-2", { selected: true }),
          ],
        })
      );

      expect(sessions.completeApply).toHaveBeenCalledWith(
        expect.objectContaining({
          ...
          reviewSnapshot: expect.objectContaining({
            ...
            applied: expect.arrayContaining([
              expect.objectContaining({ id: "task-existing", operationId: "op-1" }),
              expect.objectContaining({ operationId: "op-2", type: "note" }),
            ]),
          }),
        })
      );
