...
import { ActiveNoteApplyError, assertApplyPayloadMatchesKind } from "@spydr/active-notes";
import { buildReviewSnapshot, mergeReviewSnapshot } from "@spydr/active-notes";
import type { IActiveNoteSessionRepository } from "@spydr/active-notes";
import type { SpydrPriority } from "../../shared/models/shared.js";
...
import { withoutProjectEmbeddingRefresh } from "../../shared/application/embedding-aware-command-bus.js";
import {
...
import { CreatePersonCommand } from "../../people/commands/index.js";
  ...
  implements ICommandHandler<ApplyActiveNoteCommand, ActiveNoteApplyResult>
{
  ...
  async execute(command: ApplyActiveNoteCommand): Promise<ActiveNoteApplyResult> {
    return withoutProjectEmbeddingRefresh(() => this.executeApply(command));
  }

  private async executeApply(
    command: ApplyActiveNoteCommand
  ): Promise<ActiveNoteApplyResult> {
    ...
    if (sessionId && this.sessions) {
      try {
        const existing = await this.sessions.getForUser({
          sessionId,
          organizationId: command.orgId,
          userId: command.userId,
        });
        previousSnapshot = existing?.reviewSnapshot ?? null;
      } catch (error) {
        console.error("[active-note.apply] load previous snapshot failed", error);
      }
    }

    const reviewSnapshot = mergeReviewSnapshot(
      previousSnapshot,
      buildReviewSnapshot({
